...
# Year file will be at most 320 airports x 12 months x 7 days x 24 hours = 645,120 rows. Will have an origin and departure file for each year.
...
if(!file.exists(file.path(sharedloc, 'Congestion_2015-2018.RData'))){
  ...
  # Sequential loop for year
  for(yr in unique(d_crossyear$YEAR)){
    # yr = '2015'
    ...
    dy_o <- dy %>% 
      group_by(ORIGIN, MONTH, DAY_OF_WEEK, DEP_TIME_BLK) %>%
    ...
    dy_o <- left_join(dy_o, dy_ox) %>%
      mutate(Relative_Congestion = N_flights / N_flights_max) %>%
      select(-N_flights_max)
    
    assign(paste0('Congestion_Origin_', yr), dy_o)
  
    dy_d <- dy %>% 
      group_by(DEST, MONTH, DAY_OF_WEEK, ARR_TIME_BLK) %>%
      summarize(N_flights = n())  
    ...
    dy_d <- left_join(dy_d, dy_dx) %>%
      mutate(Relative_Congestion = N_flights / N_flights_max) %>%
      select(-N_flights_max)
    
    assign(paste0('Congestion_Destination_', yr), dy_d)
