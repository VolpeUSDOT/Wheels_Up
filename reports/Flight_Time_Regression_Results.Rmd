...
<!--
TODO:
- Report percent error, e.g. d_across_Internal %>% mutate(pct_err = 100* (MAE/Obs_Mean)) %>% summarize(mean(pct_err, na.rm=T)) # about 5.4% error
...
- The relatively simple models here demonstrate that carriers can be reasonably expected to predict air times within a 10 minute range for the majority of flights. The median error across all flights predicted in 2019 was `r round(median(d_across_2019$MAE, na.rm=T), 2)` minutes (O-D across carriers, validation on 2019 data). `r round(d_across_2019 %>% filter(!is.na(MAE)) %>% summarize(pct_within_10 = 100*sum(MAE <= 10)/n()), 1)`% of flights were predicted within a 10 minute margin of error. 
...
- Median $r^2$ was around 0.3, which is not high. A substantial degree of variation in flight times is not captured in these models, which is again to be expected. Incorporating weather variables could substantially increase the $r^2$ for historical flight time models, but would not be useful for predicting flight time at the time of booking (except for consumers who book within a few days of departure).
- Modeling flight time across carriers rather than within each carrier yielded better performance (median MAE across = `r round(median(d_across_2019$MAE, na.rm=T), 2)` minutes; median MAE within = `r round(median(d_within_2019$MAE, na.rm=T), 2)` minutes). This indicates that there is useful information from other carriers flying the same flight segment when predicting flight time for a specific flight.
...
Two additional analyses are described here. First, relative congestion at each airport as an origin and destination was calculated by year, month, day of week, and time block. For 2019 data validation, the 2018 relative congestion values were used, sinc a complete year of data is needed to calculate relative congestion. Second, results from the machine learning approach, XGBoost, are prested and compared with the linear regression approaches.  

## O-D Across Carrier, plus congestion
...
for(across_carr_valid in valid_type){
  ols_res <- dir(file.path(resultsloc, paste0('1O-D_Congestion_Validate_', across_carr_valid)))

  fn = file.path(resultsloc, paste0('1O-D_Congestion_Validate_', across_carr_valid, '.csv'))
  if(!file.exists(fn)){
    d <- read.csv(file.path(resultsloc, paste0('1O-D_Congestion_Validate_', across_carr_valid), paste0('1O-D_Congestion_Validate_', across_carr_valid, '.csv')))
    d <- d %>%
      filter(O_D != '') %>%
      mutate(Origin = substr(O_D, 1, 3),
             Destination = substr(O_D, 5, 7))
  
    assign(paste0('d_across_cong_', across_carr_valid), d)
    
    write.csv(d, file = fn, row.names = F)
    } else {
      assign(paste0('d_across_cong_', across_carr_valid), read.csv(fn))
      }
  }
...
Compare to models without these two variables added, models without these variables performed substantially better. For flight segements which are flown less frequently, adding these variables induced substantial model overfitting, reducing the predictive power of the models.
...
#
# plot(d_across_Internal$MAE,
#      d_across_cong_Internal$MAE,
#      xlim = range(range(d_across_Internal$MAE, na.rm= T),range(d_across_cong_Internal$MAE, na.rm = T)),
#      ylim = range(range(d_across_Internal$MAE, na.rm= T),range(d_across_cong_Internal$MAE, na.rm = T)))
...
Similarly to the internal validation, the use of the congestion variables substantially weakened the predictive ability of the models, with a few extreme cases. While a handful of O-D pairs were predicted better by models with congestion variables, in general these were worse than models with just the schedule variables.
...
## O-D Across Carrier, using XGBoost

```{r xgbprep}

valid_type = c('Internal')#, '2019') #### !!! Add in 2019 when done

for(across_carr_valid in valid_type){
  ols_res <- dir(file.path(resultsloc, paste0('1O-D_Congestion_xgB_Validate_', across_carr_valid)))

  fn = file.path(resultsloc, paste0('1O-D_Congestion_xgB_Validate', across_carr_valid, '.csv'))
  if(!file.exists(fn)){
    d <- read.csv(file.path(resultsloc, paste0('1O-D_Congestion_xgB_Validate_', across_carr_valid), paste0('1O-D_Congestion_xgB_Validate_', across_carr_valid, '.csv')))
    d <- d %>%
      filter(O_D != '') %>%
      mutate(Origin = substr(O_D, 1, 3),
             Destination = substr(O_D, 5, 7))
    ...
    write.csv(d, file = fn, row.names = F)
      ...
      assign(paste0('d_across_xgb_', across_carr_valid), read.csv(fn))
      }
  }
...
The XGBoost models marginally improved model performance for flight segments with fewer than 1,000 segments flown. Namely, MAE of XGBoost models was lower than that of linear regression models, i.e. to the right of the 1:1 line in the plot below.  However, for O-D pairs of 1,000 or more flights in the training data, XGBoost models did not improve model performance.  
...
```{r airport_summary_ext_xgb, eval=FALSE, include=FALSE}
d_airport <- d_across_2019 %>%
  group_by(Origin) %>%
  filter(!is.na(MAE)) %>%
  summarize(N_flight_segments = n(),
