...
- Adding congestion variables improved the predictive ability of the models substantially. Across all 5,101 O-D pairs modeled, congestion variables were included in all except for 915 of the final models.

- The machine learning algorithm XGBoost provided the best estimates of flight time for a minority of O-D pairs, 782 using the 2019 flight data validation set. Within the XGBoost models, the most important variables were typically the relative congestion at the destination airport (297 O-D pairs) and relative congestion at the origin airport (242 O-D pairs).

- Of all 5,101 O-D pairs, there was insufficient data on 101 flight segments when validating with across all available data (2015-2018). Using just the 2019 flight data available to date, 1,077 O-D pairs had insufficient data. 
...
ggplot(d_ensemble_Internal) +
  ...
  ggtitle('Model error for each O-D pair validated on 2019 data. \n Size of points represents number of flights.')
...
ggplot(d_ensemble_Internal %>% mutate(pct_Err = 100*(MAE / Obs_Mean))) +
  ...
  ggtitle('Percent model error for each O-D pair validated on 2019 data. \n Size of points represents number of flights.')
